    ...
    handleInteraction: async (interaction) => {
        ...
        if (interaction.customId === 'farm_abrir_pasta') {
            try {
                ...
                const componentes = msg.components.map(
                    row => { row.components = row.components.map
                        (btn => {
                            if (btn.data.custom_id === 'farm_abrir_pasta') { btn.data.label = 'Pasta criada ✅'; } 
                            return btn; }); 
                            return row; }); 
                await msg.edit({ components }); 
                await interaction.editReply({ content: Sua pasta foi criada: ${pasta} }); } 
            catch (err) { console.error('Erro ao abrir pasta:', err); 
        try { if (!interaction.replied && !interaction.deferred) 
